    ...
    db.open(function(err, db) {
        db.collection('expenses', function(err, collection) {
            ...
            if (req.query.full != 'yes') {
                ...
                var ty = req.query.ty ? req.query.ty : y;
                var tm = req.query.td ? (req.query.tm ? req.query.tm : m) : (Number(m) + 1);
                var td = req.query.td ? req.query.td : '01';
                var end = toDate(td + '/' + tm + '/' + ty);
                search_query.date = {
                    $gte: start,
                    $lt: end
                };
